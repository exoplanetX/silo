# Task: SILO-DOS v0.4 Local Mirror Smoke Test

## Metadata

- Task ID: `20260607-03-01`
- Slug: `v04-smoke-test`
- Date: 2026-06-07
...
- Expected report: `tasks/reports/20260607-03-01_v04-smoke-test_report.md`

## Objective

Verify that the upgraded local `silo-development-operator` v0.4 can use `.silo-dos/` as
the primary decision mirror and produce the correct current project decision without
modifying solver functionality.
...
```text
Confirm v0.4 local mirror decision behavior from the current repository state.
```
...
- Mode A would not issue solver, native, Phase 9 closure, or Phase 10 work by default;
- Remote Sync Proof is included in the final digest;
- the standing approval profile is recognized but bounded by `technical_route` and
  `remote_sync_proof`.

## Allowed Changes

- Create this matching task file under `tasks/codex/`.
- Create `tasks/reports/20260607-03-01_v04-smoke-test_report.md`.
